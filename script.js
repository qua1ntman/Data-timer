...
function dateNow (date) {
    ...
    let s = allS%60
    let m = Math.ceil((allS/60)%60)
    let h =  Math.ceil((allS/60/60)%24)
    let d =  Math.ceil((allS/60/60/24)%30)
    let mth =  Math.floor((allS/60/60/24/30)%12)
    let y =  Math.floor(allS/60/60/24/30/12)
    ...
    // Добавляем контент
    ymdDiv.innerHTML =`D: ${d} / M: ${mth} / Y: ${y}`
    hourDiv.innerHTML = h.toString().length === 1 ? '' + '0' + h : h
    minuteDiv.innerHTML = m.toString().length === 1 ? '' + '0' + m : m
    secondDiv.innerHTML = s.toString().length === 1 ? '' + '0' + s : s
...
function chosenDate () {
    ...
    const monthInput = parseInt(document.querySelector('#input-month').value, 10)-1
    ...
    if (new Date(yearInput, monthInput, dayInput, hourInput, minuteInput, secondInput) - new Date() <= 0) {
    ...
    // Первая секунда
    dateNow(new Date(yearInput, monthInput, dayInput, hourInput, minuteInput, secondInput))
